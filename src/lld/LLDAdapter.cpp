...
void LLDAdapter::Impl::processLibrary(const std::string& libraryPath)
{
   ...
   // Check if already processed
   if (processedLibraries.find(libraryPath) != processedLibraries.end())
   {
      return;
   }

   processedLibraries.insert(libraryPath);
   if (verbose)
   {
...
const std::set<std::string>& LLDAdapter::getProcessedLibraries() const
{
...
bool LLDAdapter::shouldProcessFile(const std::string& filePath) const
{
   ...
   return extension == ".o" || extension == ".obj" || extension == ".a" || extension == ".so" ||
          extension == ".dylib" || extension == ".dll" || extension == ".exe" ||
          extension.empty();  // Accept files without extension (Linux executables)
}
