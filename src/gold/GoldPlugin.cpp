...
int heimdall_set_output_path(const char* path)
{
    if (path) {
        ...
        if (globalAdapter) globalAdapter->setOutputPath(outputPath);
        if (verbose) {
...
int heimdall_set_format(const char* fmt)
{
    if (fmt) {
        ...
        if (globalAdapter) globalAdapter->setFormat(format);
        if (verbose) {
...
{
    verbose = v;
    if (globalAdapter) globalAdapter->setVerbose(v);
}
...
int heimdall_process_input_file(const char* filePath)
{
    ...
    for (const auto& dep : deps) {
        ...
        if (!depPath.empty() && std::filesystem::exists(depPath)) {
            ...
            if (std::find(processedLibraries.begin(), processedLibraries.end(), depPath) == processedLibraries.end()) {
                processedLibraries.push_back(depPath);
                if (verbose) {
                    std::cout << "Heimdall: Auto-processing dependency library: " << depPath << std::endl;
                }
    ...
    if (verbose) {
        std::cout << "Heimdall: Processed file: " << fileName << " (checksum: " << checksum << ", size: " << fileSize << ")" << std::endl;
    }
...
{
    if (!globalAdapter || !libraryPath) return -1;
    ...
    if (verbose) {
        std::cout << "Heimdall: Processed library: " << fileName << " (checksum: " << checksum << ", size: " << fileSize << ")" << std::endl;
    }
...
void heimdall_finalize()
{
    ...
    if (format != "spdx" && format != "cyclonedx") {
        std::cerr << "Heimdall Gold Plugin: Invalid format '" << format << "', defaulting to SPDX" << std::endl;
        format = "spdx";
    }
