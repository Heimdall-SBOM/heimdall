...
class MetadataExtractionBenchmark
{
   ...
   void run()
   {
      ...
      heimdall::performance_utils::run_benchmark("Metadata Extraction", benchmark_func,
                                                 iterations_);
   }
};
      ...
      for (int i = 0; i < iterations_; ++i)
      {
         ...
         try
         {
            ...
            if (extractor.extractMetadata(component))
            {
               ...
               std::cout << "  Memory before: " << (memory_before / (1024 * 1024)) << " MB"
                         << std::endl;
               std::cout << "  Memory after: " << (memory_after / (1024 * 1024)) << " MB"
                         << std::endl;
               std::cout << "  Memory delta: " << (memory_delta / (1024 * 1024)) << " MB"
                         << std::endl;
...
class PerformanceTestSuite
{
   ...
   void run_all_tests()
   {
      ...
      std::cout << "Memory tracking enabled: " << (enable_memory_tracking_ ? "Yes" : "No")
                << std::endl;
...
void print_usage(const char* program_name)
{
   ...
   std::cout
      << "  --output <file>      Output file for results (default: heimdall_benchmark_results.json)"
      << std::endl;
   std::cout << "  --help               Show this help message" << std::endl;
   std::cout << std::endl;
   std::cout << "Examples:" << std::endl;
   std::cout << "  " << program_name << " /path/to/test/binary" << std::endl;
   std::cout << "  " << program_name << " --iterations 5 --output results.json /path/to/test/binary"
             << std::endl;
