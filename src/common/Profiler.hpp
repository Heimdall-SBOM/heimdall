...
class PerformanceSession
{
  ...
  const MemoryTracker& get_memory_tracker() const
  {
...
class Profiler
{
  ...
  void print_summary()
  {
    ...
    for (const auto& session : completed_sessions_)
    {
      ...
      std::cout << "  Peak Memory: " << session->get_memory_tracker().get_peak_memory() << " bytes"
                << std::endl;
    ...
    std::cout << "Average time per session: " << (total_time / completed_sessions_.size()) << "s"
              << std::endl;
  }
};
