...
bool MetadataExtractor::extractMetadata(ComponentInfo& component)
{
  try
  {
    ...
    {
      std::cerr
        << "[DEBUG] MetadataExtractor: Detected Mach-O file, calling enhanced extraction: " +
             component.filePath
        << std::endl;
    ...
    {
      std::cerr << "[DEBUG] MetadataExtractor: File is not Mach-O: " + component.filePath
                << std::endl;
...
bool MetadataExtractor::extractEnhancedMachOMetadata(ComponentInfo& component)
{
  if (!isMachO(component.filePath))
  {
    std::cerr << "[DEBUG] Enhanced Mach-O extraction: File is not a Mach-O binary: " +
                   component.filePath
              << std::endl;
  ...
  std::cerr << "[DEBUG] Enhanced Mach-O extraction: Starting extraction for: " + component.filePath
            << std::endl;
  ...
  std::cerr << "[DEBUG] Code signing extraction: " << (codeSignSuccess ? "success" : "failed")
            << std::endl;
  anySuccess |= codeSignSuccess;

  bool buildConfigSuccess = extractMachOBuildConfig(component);
  std::cerr << "[DEBUG] Build config extraction: " << (buildConfigSuccess ? "success" : "failed")
            << std::endl;
  ...
  std::cerr << "[DEBUG] Architectures extraction: " << (architecturesSuccess ? "success" : "failed")
            << std::endl;
  ...
  if (anySuccess)
  {
    ...
    if (appBundleSuccess && !component.version.empty())
    {
      ...
      {
        std::cerr << "[DEBUG] Updated component version from Info.plist to: " << component.version
                  << std::endl;
      }
      else
      {
        std::cerr << "[DEBUG] Component version confirmed from Info.plist: " << component.version
                  << std::endl;
      }
    ...
    if (!versionSetFromInfoPlist)
    {
      ...
      if (!component.buildConfig.sourceVersion.empty())
      {
        ...
        std::cerr << "[DEBUG] Updated component version to: " << component.buildConfig.sourceVersion
                  << std::endl;
      }
      else if (!component.buildConfig.buildVersion.empty())
      {
        component.version = component.buildConfig.buildVersion;
        std::cerr << "[DEBUG] Updated component version to: " << component.buildConfig.buildVersion
                  << std::endl;
      }
  ...
  std::cerr << "[DEBUG] Enhanced Mach-O extraction: Completed with success: " << anySuccess
            << std::endl;
                                        ...
                                        std::vector<std::string>& aliFiles)
{
  ...
  defined(HEIMDALL_CPP23_AVAILABLE)
    ...
    try
    {
      ...
      while (!dirs_to_scan.empty())
      {
        ...
        catch (const std::filesystem::filesystem_error& e)
        {
          ...
          std::cerr << "DEBUG: Skipping problematic directory: " << current_dir << ": " << e.what()
                    << std::endl;
          continue;
        }
      }
    ...
    {
      std::cerr << "DEBUG: Error searching for ALI files in: " << directory << ": " << e.what()
                << std::endl;
      return false;
    }
#else
