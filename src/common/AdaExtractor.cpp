...
AdaExtractor::AdaExtractor()
{
  ...
  securityFlags = {"NO_EXCEPTION_HANDLERS",   "NO_EXCEPTIONS",          "NO_DEFAULT_INITIALIZATION",
                                      ...
                                      const std::vector<std::string>& aliFiles)
{
  ...
  if (verbose)
  {
    ...
    if (extractEnhancedMetadata)
    {
      std::cout << ", " << allCrossRefs.size() << " cross-references, " << allTypes.size()
                << " types, " << allSecurityFlags.size() << " security flags";
    }
    std::cout << std::endl;
  }
...
bool AdaExtractor::parseAliFile(const std::string& aliFilePath, AdaPackageInfo& packageInfo)
{
  ...
  while (std::getline(file, line))
  {
    ...
    else if (line.substr(0, 2) == "X " && line.find("a*") != std::string::npos)
    {
      ...
      packageInfo.variables.insert(packageInfo.variables.end(), variables.begin(), variables.end());
      hasValidContent = true;
    }
...
bool AdaExtractor::findAliFiles(const std::string& directory, std::vector<std::string>& aliFiles)
{
  ...
  defined(HEIMDALL_CPP23_AVAILABLE)
    ...
    try
    {
      ...
      while (!dirs_to_scan.empty())
      {
        ...
        catch (const std::filesystem::filesystem_error& e)
        {
          ...
          if (verbose)
          {
            std::cerr << "AdaExtractor: Skipping problematic directory: " << current_dir << ": "
                      << e.what() << std::endl;
          }
          continue;
        }
    ...
    try
    {
      ...
      if (verbose)
      {
        std::cerr << "AdaExtractor: Starting portable directory scan for: " << directory
                  << std::endl;
      }
...
std::string AdaExtractor::extractPackageName(const std::string& aliFilePath)
{
  ...
  std::string filename =
    (lastSlash != std::string::npos) ? aliFilePath.substr(lastSlash + 1) : aliFilePath;
#endif
