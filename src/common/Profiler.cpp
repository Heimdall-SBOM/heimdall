...
public:
    ...
    void run() {
        ...
        std::cout << "Running benchmark: " << name_ << " (" << iterations_ << " iterations)" << std::endl;
        ...
        for (int i = 0; i < iterations_; ++i) {
            ...
            std::cout << "  Iteration " << (i + 1) << ": " 
                     << std::fixed << std::setprecision(6) << timer.elapsed_seconds() << "s, "
                     << memory_delta << " bytes" << std::endl;
        }
    }
    ...
    const std::vector<size_t>& get_memory_usage() const { return memory_usage_; }
};
...
public:
    ...
    static size_t get_deallocation_count() { return deallocation_count_.load(); }
    ...
    static void print_statistics() {
        ...
        std::cout << "Leaked memory: " << (get_total_allocated() - get_deallocation_count()) << " bytes" << std::endl;
    }
};
...
public:
    ...
    double get_cpu_usage() {
        ...
        if (elapsed < 100) { // Don't check too frequently
...
void print_system_memory_info() {
    ...
    std::cout << "Available system memory: " << (available_memory / (1024 * 1024)) << " MB" << std::endl;
    std::cout << "Used system memory: " << (used_memory / (1024 * 1024)) << " MB" << std::endl;
    std::cout << "Current process memory: " << (current_process_memory / (1024 * 1024)) << " MB" << std::endl;
