...
get_compiler_major_version() {
    ...
    elif [[ "$version_string" =~ \(Ubuntu[[:space:]]+([0-9]+\.[0-9]+) ]]; then
        # Ubuntu GCC format like "gcc (Ubuntu 11.4.0-1ubuntu1~22.04) 11.4.0"
        echo "$version_string" | sed -n 's/.*(Ubuntu[[:space:]]\+\([0-9]\+\.[0-9]\+\).*/\1/p' | cut -d'.' -f1
    elif [[ "$version_string" =~ [0-9]+\.[0-9]+ ]]; then
        # Any version format with numbers like "clang version 18.1.8"
        # Extract the first version number found
        echo "$version_string" | grep -o '[0-9]\+\.[0-9]\+' | head -1 | cut -d'.' -f1
