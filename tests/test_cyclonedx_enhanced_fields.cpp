...
{
   std::vector<std::string> mimeTypes = {"application/x-executable",
   ...
   for (const auto& mimeType : mimeTypes)
   {
      ...
      SBOMGenerator generator;
      generator.setOutputPath(
         (test_dir / ("mime_" + std::to_string(mimeTypes.size()) + ".json")).string());
      generator.setFormat("cyclonedx");
      generator.setCycloneDXVersion("1.6");
      generator.processComponent(component);
...
{
   std::vector<std::string> validCPEs = {"cpe:2.3:a:example:test-component:1.0.0:*:*:*:*:*:*:*:*",
...
TEST_F(CycloneDXEnhancedFieldsTest, ExternalReferences)
{
   ...
   std::vector<std::string> externalRefs = {
      "https://example.com/test-component", "https://docs.example.com/test-component",
   ...
   component.addProperty("external:download",
                         "https://download.example.com/test-component-1.0.0.tar.gz");
   ...
   for (const auto& prop : componentJson["properties"])
   {
      ...
      else if (name == "external:documentation" &&
               value == "https://docs.example.com/test-component")
      {
         foundDocs = true;
      }
      else if (name == "external:download" &&
               value == "https://download.example.com/test-component-1.0.0.tar.gz")
      {
         foundDownload = true;
      }
   }
