...
char* concatenate_strings(const char* str1, const char* str2) {
    ...
    void compileTestProject() {
        ...
        std::string string_obj_cmd = "gcc -c -g3 -O0 -fno-omit-frame-pointer -Wall -Wextra -o " + 
                                    string_object.string() + " " + string_source.string() + " 2>/dev/null";
        ...
        std::string math_lib_cmd = "gcc -shared -fPIC -g3 -O0 -fno-omit-frame-pointer -Wall -Wextra -o " + 
                                  math_library.string() + " " + math_source.string() + " 2>/dev/null";
        system(math_lib_cmd.c_str());
        
        string_library = test_dir / "libstring_utils.so";
        std::string string_lib_cmd = "gcc -shared -fPIC -g3 -O0 -fno-omit-frame-pointer -Wall -Wextra -o " + 
        ...
        if (result) {
            ...
            if (component.containsDebugInfo) {
                ...
                for (const auto& expected : expected_sources) {
                    ...
                    EXPECT_TRUE(found) << "Expected source file '" << expected << "' not found in SBOM";
                }
            }
        }
    }
...
TEST_F(DWARFIntegrationTest, MultiComponentSBOMGeneration) {
    ...
    for (size_t i = 0; i < component_paths.size(); ++i) {
        if (std::filesystem::file_size(component_paths[i]) > 100) {
            ...
            std::cout << "  - checksum empty: " << (component.checksum.empty() ? "true" : "false") << std::endl;
