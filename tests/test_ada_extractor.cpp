...
TEST_F(AdaExtractorTest, ExtractAdaMetadata_MultipleAliFiles)
{
  try
  {
    ...
    if (!ali1_exists)
    {
      std::cerr << "ERROR: File " << ali1 << " was not created after " << max_retries << " attempts"
                << std::endl;
    ...
    if (!ali2_exists)
    {
      std::cerr << "ERROR: File " << ali2 << " was not created after " << max_retries << " attempts"
                << std::endl;
