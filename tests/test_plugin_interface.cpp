...
TEST_F(PluginInterfaceTest, ShouldProcessFileSystemLibraries)
{
  ...
  std::vector<std::string> possible_libc_paths = {
    "/usr/lib/libc.so", "/usr/lib64/libc.so", "/usr/lib/x86_64-linux-gnu/libc.so",
    "/lib/x86_64-linux-gnu/libc.so", "/lib64/libc.so"};
  ...
  std::vector<std::string> possible_libstdcxx_paths = {"/usr/lib64/libstdc++.so",
...
TEST_F(PluginInterfaceTest, ProcessSymbolWithoutComponent)
{
  ...
  {
    std::cerr << "ERROR: Exception in ProcessSymbolWithoutComponent test: " << e.what()
              << std::endl;
    throw;
  }
  catch (...)
