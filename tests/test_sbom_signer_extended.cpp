...
class SBOMSignerExtendedTest : public ::testing::Test
{
   ...
   nlohmann::json createTestSBOM()
   {
      ...
      sbom["version"]     = 1;
      sbom["metadata"]    = {{"timestamp", "2025-07-28T10:00:00Z"},
...
TEST_F(SBOMSignerExtendedTest, LargeSBOMPerformance)
{
   ...
   EXPECT_LT(duration.count(), 5000)
      << "Signing large SBOM took too long: " << duration.count() << "ms";
...
TEST_F(SBOMSignerExtendedTest, ConcurrentSigningOperations)
{
   ...
   for (int i = 0; i < numThreads; ++i)
   {
      threads.emplace_back(
         [this, i, operationsPerThread, &results]()
         {
            SBOMSigner signer;
            ...
            for (int j = 0; j < operationsPerThread; ++j)
            {
               ...
               sbom["metadata"]["timestamp"] =
                  "2025-07-28T" + std::to_string(i) + ":" + std::to_string(j) + ":00Z";
...
TEST_F(SBOMSignerExtendedTest, SpecialCharactersInSBOM)
{
   ...
   EXPECT_TRUE(signResult) << "Failed to sign SBOM with special characters: "
                           << signer.getLastError();
   EXPECT_EQ(signatureInfo.algorithm, "RS256");
   EXPECT_FALSE(signatureInfo.signature.empty());
   ...
   ASSERT_TRUE(signer.loadPublicKey(rsa_public_key));
   bool verifyResult = signer.verifySignature(sbomContent);
   EXPECT_TRUE(verifyResult) << "Failed to verify SBOM with special characters: "
                             << signer.getLastError();
}
...
TEST_F(SBOMSignerExtendedTest, DeeplyNestedStructures)
{
   ...
   EXPECT_TRUE(signResult) << "Failed to sign SBOM with nested structures: "
                           << signer.getLastError();
   EXPECT_EQ(signatureInfo.algorithm, "RS256");
   EXPECT_FALSE(signatureInfo.signature.empty());
   ...
   ASSERT_TRUE(signer.loadPublicKey(rsa_public_key));
   bool verifyResult = signer.verifySignature(sbomContent);
   EXPECT_TRUE(verifyResult) << "Failed to verify SBOM with nested structures: "
                             << signer.getLastError();
}
...
TEST_F(SBOMSignerExtendedTest, LargeArrays)
{
   ...
   for (int i = 0; i < 1000; ++i)
   {
      nlohmann::json hash;
      hash["alg"]     = "SHA-256";
      hash["content"] = "hash" + std::to_string(i) +
   ...
   ASSERT_TRUE(signer.loadPublicKey(rsa_public_key));
   bool verifyResult = signer.verifySignature(sbomContent);
   EXPECT_TRUE(verifyResult) << "Failed to verify SBOM with large arrays: "
                             << signer.getLastError();
}
...
TEST_F(SBOMSignerExtendedTest, MixedDataTypes)
{
   ...
   sbom["metadata"]["mixedData"] = {
      ...
      {"object",
       {{"nested", "value"}, {"numbers", {1.5, 2.7, 3.14}}, {"booleans", {true, false, true}}}}};
   ...
   EXPECT_TRUE(signResult) << "Failed to sign SBOM with mixed data types: "
                           << signer.getLastError();
   EXPECT_EQ(signatureInfo.algorithm, "RS256");
   EXPECT_FALSE(signatureInfo.signature.empty());
   ...
   ASSERT_TRUE(signer.loadPublicKey(rsa_public_key));
   bool verifyResult = signer.verifySignature(sbomContent);
   EXPECT_TRUE(verifyResult) << "Failed to verify SBOM with mixed data types: "
                             << signer.getLastError();
}
...
TEST_F(SBOMSignerExtendedTest, FloatingPointNumbers)
{
   ...
   sbom["metadata"]["floatingPointData"] = {
      {"pi", 3.14159265359},    {"e", 2.71828182846}, {"sqrt2", 1.41421356237},
      {"negative", -1.5},       {"zero", 0.0},        {"large", 1.23456789e+10},
   ...
   EXPECT_TRUE(signResult) << "Failed to sign SBOM with floating point numbers: "
                           << signer.getLastError();
   EXPECT_EQ(signatureInfo.algorithm, "RS256");
   EXPECT_FALSE(signatureInfo.signature.empty());
   ...
   ASSERT_TRUE(signer.loadPublicKey(rsa_public_key));
   bool verifyResult = signer.verifySignature(sbomContent);
   EXPECT_TRUE(verifyResult) << "Failed to verify SBOM with floating point numbers: "
                             << signer.getLastError();
}
...
TEST_F(SBOMSignerExtendedTest, EmptyObjectsAndArrays)
{
   ...
   sbom["metadata"]["nestedEmpty"] = {{"empty1", nlohmann::json::object()},
   ...
   EXPECT_TRUE(signResult) << "Failed to sign SBOM with empty structures: "
                           << signer.getLastError();
   EXPECT_EQ(signatureInfo.algorithm, "RS256");
   EXPECT_FALSE(signatureInfo.signature.empty());
   ...
   ASSERT_TRUE(signer.loadPublicKey(rsa_public_key));
   bool verifyResult = signer.verifySignature(sbomContent);
   EXPECT_TRUE(verifyResult) << "Failed to verify SBOM with empty structures: "
                             << signer.getLastError();
}
...
TEST_F(SBOMSignerExtendedTest, DifferentJSONEncodings)
{
   ...
   for (size_t i = 0; i < jsonEncodings.size(); ++i)
   {
      ...
      EXPECT_TRUE(signResult) << "Failed to sign SBOM with encoding " << i << ": "
                              << signer.getLastError();
      EXPECT_EQ(signatureInfo.algorithm, "RS256");
      EXPECT_FALSE(signatureInfo.signature.empty());
      ...
      EXPECT_TRUE(verifyResult) << "Failed to verify SBOM with encoding " << i << ": "
                                << signer.getLastError();
   }
}
...
TEST_F(SBOMSignerExtendedTest, DifferentKeySizes)
{
   ...
   for (int keySize : keySizes)
   {
      ...
      std::string privateKeyPath =
         (test_dir / ("rsa_" + std::to_string(keySize) + "_private.key")).string();
      std::string publicKeyPath =
         (test_dir / ("rsa_" + std::to_string(keySize) + "_public.key")).string();

      std::string genCmd =
         "openssl genrsa -out " + privateKeyPath + " " + std::to_string(keySize) + " 2>/dev/null";
      system(genCmd.c_str());
      std::string pubCmd =
      ...
      SBOMSigner signer;
      ASSERT_TRUE(signer.loadPrivateKey(privateKeyPath))
         << "Failed to load " << keySize << "-bit private key";
      ...
      EXPECT_TRUE(signResult) << "Failed to sign with " << keySize
                              << "-bit key: " << signer.getLastError();
      EXPECT_EQ(signatureInfo.algorithm, "RS256");
      EXPECT_FALSE(signatureInfo.signature.empty());
      ...
      // Verify signature
      ASSERT_TRUE(signer.loadPublicKey(publicKeyPath))
         << "Failed to load " << keySize << "-bit public key";
      bool verifyResult = signer.verifySignature(sbomContent);
      EXPECT_TRUE(verifyResult) << "Failed to verify with " << keySize
                                << "-bit key: " << signer.getLastError();
   }
}