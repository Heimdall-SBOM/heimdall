...
function(heimdall_get_target_sbom_properties target format output verbose include_system_libs name)
    ...
    # Get format
    get_target_property(target_format ${target} HEIMDALL_SBOM_FORMAT)
    if(target_format)
        set(${format} ${target_format} PARENT_SCOPE)
    else()
        set(${format} ${HEIMDALL_SBOM_FORMAT} PARENT_SCOPE)
    endif()
    
    # Get output
    get_target_property(target_output ${target} HEIMDALL_SBOM_OUTPUT)
    if(target_output)
        set(${output} ${target_output} PARENT_SCOPE)
    else()
        # Generate default output path
        heimdall_get_format_version(${format} format_type format_version)
        if(format_type STREQUAL "spdx")
            set(${output} "${HEIMDALL_SBOM_OUTPUT_DIR}/${target}.spdx" PARENT_SCOPE)
        else()
            set(${output} "${HEIMDALL_SBOM_OUTPUT_DIR}/${target}.cyclonedx.json" PARENT_SCOPE)
        endif()
    endif()
    
    # Get verbose setting
    get_target_property(target_verbose ${target} HEIMDALL_SBOM_VERBOSE)
    if(target_verbose)
        set(${verbose} ${target_verbose} PARENT_SCOPE)
    else()
        set(${verbose} ${HEIMDALL_VERBOSE} PARENT_SCOPE)
    endif()
    
    # Get system libs setting
    get_target_property(target_include_system ${target} HEIMDALL_SBOM_INCLUDE_SYSTEM_LIBS)
    if(target_include_system)
        set(${include_system_libs} ${target_include_system} PARENT_SCOPE)
    else()
        set(${include_system_libs} ${HEIMDALL_INCLUDE_SYSTEM_LIBS} PARENT_SCOPE)
    endif()
    
    # Get name
    get_target_property(target_name ${target} HEIMDALL_SBOM_NAME)
    if(target_name)
        set(${name} ${target_name} PARENT_SCOPE)
    else()
        set(${name} ${target} PARENT_SCOPE)
    endif()
endfunction()
