#!/usr/bin/env python3
"""
Validation script for CycloneDX 1.5 compliance fix.
This script validates the JSON structure against the key CycloneDX 1.5 schema requirements.
"""
...
class CycloneDX15Validator:
    """Validator for CycloneDX 1.5 schema compliance"""
    ...
    def validate_sbom_structure(self, sbom: Dict[Any, Any]) -> bool:
        """Validate the main SBOM structure against CycloneDX 1.5 requirements"""
        is_valid = True
        ...
        if sbom.get("specVersion") != "1.5":
            self.errors.append(f"Invalid specVersion: {sbom.get('specVersion')}, expected '1.5'")
            is_valid = False
        
        # Check for schema reference (recommended)
        if "$schema" not in sbom:
            self.warnings.append("Missing $schema field (recommended)")
        elif not sbom["$schema"].endswith("bom-1.5.schema.json"):
    ...
    def validate_metadata(self, metadata: Dict[Any, Any]) -> bool:
        """Validate metadata structure for CycloneDX 1.5"""
        is_valid = True
        
        # Check tools structure (1.5 supports both formats)
        if "tools" in metadata:
            tools = metadata["tools"]
            if isinstance(tools, list):
                self.warnings.append("Tools array format is deprecated, use tools.components in 1.5")
    ...
    def validate_component(self, component: Dict[Any, Any], is_main: bool = False) -> bool:
        """Validate individual component structure for CycloneDX 1.5"""
        is_valid = True
        ...
        # Validate type field (1.5 doesn't have cryptographic-asset)
        valid_types_1_5 = [
            "application", "framework", "library", "container", "platform",
            "operating-system", "device", "device-driver", "firmware", "file",
            "machine-learning-model", "data"
        ]
        if "type" in component and component["type"] not in valid_types_1_5:
            self.errors.append(f"Invalid component type for 1.5: {component['type']}")
            is_valid = False
        ...
        # Validate evidence structure (1.5 feature)
        if "evidence" in component:
            is_valid &= self.validate_evidence_1_5(component["evidence"])
    ...
    def validate_evidence_1_5(self, evidence: Dict[Any, Any]) -> bool:
        """Validate evidence structure for CycloneDX 1.5"""
        is_valid = True
        
        # Evidence structure validation - check for proper 1.5 fields
        ...
        # Check callstack structure (1.5 specific - requires module field)
        if "callstack" in evidence:
            callstack = evidence["callstack"]
            if "frames" in callstack:
                for frame in callstack["frames"]:
                    if "module" not in frame:
                        self.errors.append("CycloneDX 1.5 callstack frames must have 'module' field")
                        is_valid = False
        
        return is_valid
    
    def generate_sample_valid_cyclonedx_15(self) -> str:
        """Generate a sample valid CycloneDX 1.5 SBOM for comparison"""
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python3 validate_cyclonedx_1.5_fix.py <sbom_file_or_json_content>")
        print("       python3 validate_cyclonedx_1.5_fix.py --sample")
        sys.exit(1)
    
    validator = CycloneDX15Validator()
    
    if sys.argv[1] == "--sample":
        print("Generating sample valid CycloneDX 1.5 SBOM:")
        print(validator.generate_sample_valid_cyclonedx_15())
        return
    ...
    print("Validating CycloneDX 1.5 compliance...")
