...
void testEnhancedAdaExtraction()
{
  ...
  if (extractor.extractAdaMetadata(component, aliFiles))
  {
    ...
    for (const auto& [key, value] : component.properties)
    {
      if (key == "security.buildFlags")
        hasSecurityFlags = true;
      if (key == "functions.calls")
        hasCallGraph = true;
      if (key == "types.variables")
        hasTypeInfo = true;
      if (key == "build.timestamps")
        hasTimestamps = true;
      if (key == "build.checksums")
        hasChecksums = true;
      if (key == "security.runtimeFlags")
        hasRuntimeFlags = true;
      if (key == "security.compilerVersion")
        hasCompilerVersion = true;
    }
...
void testIndividualALIFileParsing()
{
  ...
  for (const auto& aliFile : aliFiles)
  {
    ...
    if (extractor.parseAliFile(aliFile, packageInfo))
    {
      ...
      std::cout << "Is Specification: " << (packageInfo.isSpecification ? "Yes" : "No")
                << std::endl;
  ...
  for (const auto& aliFile : aliFiles)
  {
    ...
    if (file.is_open())
    {
      ...
      if (extractor.extractTypeInfo(content, types))
      {
        ...
        {
          std::cout << "  " << type.name << " (base: " << type.baseType << ", size: " << type.size
                    << ", alignment: " << type.alignment << ")" << std::endl;
        }
      }
      ...
      if (extractor.extractBuildInfo(content, buildInfo))
      {
        ...
        std::cout << "  Optimization Flags (" << buildInfo.optimizationFlags.size()
                  << "):" << std::endl;
