...
# CycloneDX-only Heimdall Usage Example Script
# This script demonstrates how to use Heimdall to generate only CycloneDX SBOMs
...
if [[ ! -f "main.cpp" ]]; then
    print_error "main.cpp not found. Please run this script from the heimdall-usage-cyclonedx-example directory."
    exit 1
fi

print_status "Starting CycloneDX-only Heimdall usage example..."

# Clean up any previous builds
print_status "Cleaning previous builds..."
rm -f *.o app-lld app-gold *.cyclonedx.json
...
print_status "Step 2: Linking with LLD and generating CycloneDX SBOM using wrapper approach..."
    ...
    if [[ -f "$HEIMDALL_BUILD_DIR/lib/heimdall-lld.so" ]]; then
        if g++ -fuse-ld=lld main.o utils.o math.o -o app-lld; then
            ...
            if [[ -x "$HEIMDALL_SBOM_PATH" ]]; then
                print_status "Generating CycloneDX SBOM using heimdall-sbom wrapper..."
                "$HEIMDALL_SBOM_PATH" ../../build-cpp23/lib/heimdall-lld.so app-lld --format cyclonedx --output app-lld.cyclonedx.json && print_success "CycloneDX SBOM generated: app-lld.cyclonedx.json" || print_warning "CycloneDX SBOM generation failed."
            else
    ...
    if [[ -f "$HEIMDALL_BUILD_DIR/lib/heimdall-gold.so" ]]; then
        ...
        if g++ -fuse-ld=gold -Wl,--plugin=$HEIMDALL_BUILD_DIR/lib/heimdall-gold.so \
            ...
            if g++ -fuse-ld=gold main.o utils.o math.o -o app-gold; then
                ...
                HEIMDALL_SBOM_PATH="../../build-cpp23/src/tools/heimdall-sbom"
                if [[ -x "$HEIMDALL_SBOM_PATH" ]]; then
                    print_status "Generating CycloneDX SBOM using heimdall-sbom wrapper for Gold..."
                    "$HEIMDALL_SBOM_PATH" ../../build-cpp23/lib/heimdall-gold.so app-gold --format cyclonedx --output app-gold.cyclonedx.json && print_success "CycloneDX SBOM generated: app-gold.cyclonedx.json" || print_warning "CycloneDX SBOM generation failed."
                else
                    print_warning "heimdall-sbom tool not found at $HEIMDALL_SBOM_PATH. Please ensure Heimdall is built."
                fi
            else
...
echo "=== Generated CycloneDX Files ==="
ls -la *.cyclonedx.json 2>/dev/null || print_warning "No CycloneDX files found"

echo ""
if [[ -f "app-lld.cyclonedx.json" ]]; then
    echo "LLD CycloneDX (first 10 lines):"
    head -10 app-lld.cyclonedx.json
    echo "..."
    echo ""
fi
if [[ -f "app-gold.cyclonedx.json" ]]; then
    echo "Gold CycloneDX (first 10 lines):"
    head -10 app-gold.cyclonedx.json
    echo "..."
    echo ""
fi

print_success "CycloneDX-only Heimdall usage example completed!" 